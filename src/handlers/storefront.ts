...
const PAGE_SIZE = 3;
const nav = (rows: ReturnType<typeof inlineButton>[][] = []) => inlineKeyboard([...rows, [inlineButton("⬅️ Назад", "nav:back")]]);
...
async function orders(ctx: Ctx) {
  ...
  if (!list.length) { await showPersistentMenu(ctx, { text: "У вас пока нет заказов.", markup: inlineKeyboard([[inlineButton("⬅️ Назад", "nav:back")]]) }); return; }
  ...
  rows.push([inlineButton("⬅️ Назад", "nav:back")]);
...
composer.on("message:text", async (ctx, next) => {
  ...
  if (text === "⬅️ Назад") {
    ...
    return showPersistentMenu(ctx, { text: "Добро пожаловать. Выберите раздел в меню ниже.", markup: menuKeyboard(ctx) }, { replace: true });
  }
...
composer.callbackQuery(/^shop:add:(.+)$/, async (ctx) => { await ctx.answerCallbackQuery(); const added = await addToCart(ctx, ctx.match[1]); await ctx.reply(added ? "Товар добавлен в корзину." : "Этот товар больше недоступен. Откройте каталог ещё раз."); });
composer.callbackQuery(/^catalog:cart:add:(.+)$/, async (ctx) => { await ctx.answerCallbackQuery(); const product = await getCatalogProduct(ctx.match[1]); const added = await addToCart(ctx, ctx.match[1]); await ctx.reply(added ? "Товар добавлен в корзину." : "Этот товар больше недоступен. Откройте каталог ещё раз.", { reply_markup: inlineKeyboard([[inlineButton("🛒 Открыть корзину", "shop:cart:from:" + (product?.id ?? ""))], [inlineButton("⬅️ Назад", "nav:back")]]) }); });
...
composer.callbackQuery("shop:clear", async (ctx) => { await ctx.answerCallbackQuery(); await clearCart(ctx); await ctx.reply("Корзина очищена."); });
